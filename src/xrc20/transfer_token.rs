...
/// owner address, private key, recipient address, token address, amount.
...
pub async fn transfer_token(
    ...
    let out_gas_estimate = token_contract
        .estimate_gas(
            "transfer",
            (recipient_addr, U256::exp10(18) * amount),
            owner_addr,
            Options::default(),
        )
        .await
    ...
    let data = token_contract
        ...
        .function("transfer")
        .unwrap()
        .encode_input(&(recipient_addr, U256::exp10(18) * amount).into_tokens())
        .unwrap();
